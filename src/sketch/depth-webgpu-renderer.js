...
export function createDepthWebGpuPreviewRenderer({ canvas = null, device: providedDevice = null, format = "rgba8unorm" }) {
  let device;
  let context;
  ...
  async function render({ sourceCanvas, depthCanvas, profile, settings, progress = 0.55, waitForCompletion = false }) {
    await initialize();
  ...
  function currentTargetTexture() {
    return canvas ? context.getCurrentTexture() : outputTexture;
  }

  function ensureUvBuffer(profile) {
  ...
  function uploadCanvasTexture(previousTexture, source) {
...
export function buildDepthPreviewUniformArray({ profile, settings, pose, size }) {
  const projectionMode = PROJECTION_MODE_INDEX[profile.projectionMode] ?? 0;
...
function buildUniforms({ profile, settings, pose, size }) {
  return buildDepthPreviewUniformArray({ profile, settings, pose, size });
}

function buildUvSamples(profile) {
  const values = [];
...
function splatPixelsForSettings(settings) {
