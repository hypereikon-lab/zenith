...
import type { PlateSource, RunwayOutput, ScheduleWorkspaceAutosave, VersionSnapshot, ZenithState } from "../app/types.js";
...
type VersionControllerOptions = {
  ...
  elements: {
    ...
    versionGallery: HTMLElement;
    versionReadout: HTMLElement;
    saveVersion: HTMLButtonElement;
    applyVersion: HTMLButtonElement;
    deleteVersion: HTMLButtonElement;
    exportVersion: HTMLButtonElement;
    importVersion: HTMLInputElement;
  };
  actions: {
...
export function createVersionController({
  ...
  async function saveCurrentVersion() {
    ...
    try {
      ...
      console.error(error);
      versionReadout.textContent = errorMessage(error);
    }
  }
  ...
  async function createVersionSnapshot() {
    ...
    return {
      ...
      createdAt,
      sourceNames: state.plates.slice(0, plateCount).map((plate: PlateSource) => plate.name),
      plate: {
        count: plateCount,
      ...
      generation: {
        ...
        lastOutputUrls: state.runwayOutputs.map((output: RunwayOutput) => output.url).filter(Boolean),
      },
      thumbnails: {
  ...
  function applyVersionSnapshot(version: VersionSnapshot): void {
    ...
    state.platePlacements = cloneJson(plate.patchPlacements || []).map((placement) => normalizePlatePlacement(placement));
  ...
  function deleteSelectedVersion() {
    const version = selectedVersion();
    if (!version) return;
    state.versions = state.versions.filter((item: VersionSnapshot) => item.id !== version.id);
    saveVersionsToStorage();
    updateVersionUi();
    actions.scheduleWorkspaceAutosave("version", 250);
  }
  ...
  function updateVersionUi() {
    ...
    if (state.versions.length === 0) {
      ...
      state.versions.forEach((version: VersionSnapshot) => {
        const option = document.createElement("option");
  ...
  function versionSourceMatch(version: VersionSnapshot): boolean {
    const names = version.sourceNames || [];
    if (names.length === 0 || state.plates.length < names.length) return true;
    return names.every((name: string, index: number) => state.plates[index]?.name === name);
  }
