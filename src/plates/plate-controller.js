...
export function createPlateController({ state, controls, elements, video, videoTransport, renderer, actions }) {
  const {
    commitPlateMap,
    exportPlateMap,
    platesReadout,
  ...
  async function handlePlacementEditChange() {
    updatePlateLayoutUi();
    if (controls.editPlacement.checked && !["flat", "split"].includes(state.viewMode)) {
  ...
  async function loadPlateFiles(files) {
  ...
  async function loadPlateSource(file) {
  ...
  function ensurePlatePlacements() {
  ...
  function autoArrangePlatePlacements(force) {
  ...
  function defaultPlatePlacement(index, plateCount, _plate) {
  ...
  function shouldUseDefaultPlateProfile(plateCount) {
  ...
  function schedulePlatePreview(delay) {
  ...
  function scheduleGpuPlatePreview(delay) {
    if (platePreviewTimer) {
      clearTimeout(platePreviewTimer);
    }
