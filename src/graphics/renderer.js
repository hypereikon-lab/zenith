...
export function createDomeRenderer({ dom, state, controls, video, videoTransport, viewCamera, projectionMode, actions }) {
  ...
  let floorIndexBuffer;
  let floorIndexCount = 0;
  let uniformBuffer;
  let roomUniformBuffer;
  let sampler;
  let sourceTexture;
  let plateCompositor;
  let renderSize = { width: 1, height: 1, dpr: 1 };
  let lastFrameTime = performance.now();
  let videoUploadMode = "direct";
  let videoUploadCanvas;
  ...
  function uploadMediaSource(source, width, height) {
    sourceTexture.upload(source, width, height);
  }

  function renderPlateComposite(options) {
    if (!plateCompositor) {
  ...
  async function readTextureToCanvas(texture, width, height, options = {}) {
  ...
  function uploadCurrentVideoFrame() {
  ...
  function uploadVideoFrameViaCanvas(width, height) {
  ...
  function renderToView({ width, height, colorView, depthView, depthStoreOp = "discard", submit = true }) {
  ...
  function drawDome(pass, rect) {
    setViewport(pass, rect);
    pass.setPipeline(domePipeline);
    pass.setBindGroup(0, sourceTexture.bindGroup);
    pass.setVertexBuffer(0, vertexBuffer);
    pass.setIndexBuffer(indexBuffer, "uint32");
    pass.drawIndexed(indexCount);
  }

  function drawRoom(pass, rect) {
    setViewport(pass, rect);
  ...
  function setViewport(pass, rect) {
  ...
  function writeUniforms(viewport, cutaway) {
    ...
    data[23] = projectionMode[controls.projectionMode.value] ?? 0;
  ...
  function shouldUseFastDomeMap(cutaway) {
  ...
  function writeRoomUniforms(viewport) {
    const aspect = viewport.width / viewport.height;
    const projection = perspectiveLH((Number(controls.fov.value) * Math.PI) / 180, aspect, 0.01, 20);
  ...
  function bindExternalSourceTexture(texture) {
    sourceTexture.bind(texture);
  }

  function restoreOwnedSourceTexture() {
    sourceTexture.bind();
  }

  async function captureFrame(downloadBlob) {
  ...
  async function captureCompositeCanvas(options = {}) {
    const output = await renderCaptureCanvas(options);
    drawHudToCapture(output, options);
    return output;
  }

  async function drawCompositeToCanvas(output, options = {}) {
  ...
  async function renderCaptureCanvas(options = {}) {
  ...
  function drawHudToCapture(output, options = {}) {
    const ctx = output.getContext("2d");
    if (options.square) {
