...
const TOUR_STEPS = ["flat", "orbit", "theater", "inside"] as const;
type StopOptions = { completed?: boolean };
type ViewTourState = {
  viewMode: ViewMode;
  autoRotate: boolean;
  rotation: string;
  theaterPitch: string;
  camera: ZenithState["camera"];
  videoPaused: boolean;
  videoTime: number;
};
type PlateComposeState = {
  viewMode: ViewMode;
...
type DemoTourOptions = {
  state: ZenithState;
  controls: ZenithControls;
  video: HTMLVideoElement;
  elements: {
    demoPlateCompose: HTMLButtonElement;
    demoViewTour: HTMLButtonElement;
    demoInpaintPulse: HTMLButtonElement;
    demoCursor: HTMLElement;
  ...
  actions: {
    ...
    displayTemporaryCanvas?: (canvas: HTMLCanvasElement, name: string) => void;
    ...
    onPlateComposeComplete?: () => void;
    onViewTourComplete?: () => void;
    setPulseButtonText?: (text: string) => void;
  };
};

export function createDemoTourController({ state, controls, video, elements, actions }: DemoTourOptions) {
  const { demoPlateCompose, demoViewTour, demoInpaintPulse, demoCursor, captureReadout } = elements;
  let frameId = 0;
  let startedAt = 0;
  let previousState: ViewTourState | null = null;
  let activeStepIndex = -1;
  let composeFrameId = 0;
  let composeStartedAt = 0;
  let composePreviousState: PlateComposeState | null = null;
  let composeTargetPlacements: NormalizedPlatePlacement[] = [];
  let composeStartPlacements: NormalizedPlatePlacement[] = [];
  let pulseFrameId = 0;
  let pulseStartedAt = 0;
  let pulseCanvas: HTMLCanvasElement | null = null;
  let pulseContext: CanvasRenderingContext2D | null = null;
  let pulseAfterCanvas: HTMLCanvasElement | null = null;
  ...
  function playPlateComposeDemo() {
    ...
    composeStartPlacements = composeTargetPlacements.map((placement: NormalizedPlatePlacement, index: number) =>
      stagedPlatePlacement(placement, index, composeTargetPlacements.length),
    );
  ...
  function stopPlateComposeDemo(options: StopOptions = {}) {
  ...
  function stopViewTour(options: StopOptions = {}) {
  ...
  function tickPlateCompose(now: number): void {
    ...
    state.platePlacements = composeTargetPlacements.map((target: NormalizedPlatePlacement, index: number) => {
  ...
  function applyFakeDrag(view: (typeof TOUR_STEPS)[number], progress: number, now: number): void {
  ...
  async function resolveInpaintPulseAfterCanvas(): Promise<HTMLCanvasElement | null> {
  ...
  function activeRunwayOutput(): RunwayOutput | null {
  ...
  function transitionBlend(progress: number): number {
...
function gesturePoint(progress: number, now: number): { x: number; y: number; scale: number } {
...
function stagedPlatePlacement(
  target: NormalizedPlatePlacement,
  index: number,
  count: number,
...
function moveDemoCursorForPlate(progress: number, index: number, count: number): void {
  ...
  const cursor = document.querySelector<HTMLElement>("#demoCursor");
  if (!cursor) return;
  cursor.style.left = `${x}%`;
  cursor.style.top = `${y}%`;
